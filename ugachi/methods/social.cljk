(ns ugachi.methods.social
  "social.cljc — ugachi DRY-RUN self-publication seed (ADR-2606272355, auto-seeded R0).

  This is a GENERIC self-publication projection (draft-observation-post below builds an
  app.bsky.feed.post-shaped record), mechanically derived from ugachi's own
...
(defn draft-observation-post
  "A dry-run observation post over ugachi's own domain (The §2(l) multi-generational (子・孫) × wellbecoming extraction RISK-GATE. A proposed extraction project (mine/well/quarry/brine) is…)."
...
(defn build-live
  ...
  [& _args]
  (throw (ex-info (str "ugachi R0: live social posting is Council Lv6+ + operator + "
