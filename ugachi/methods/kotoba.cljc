#!/usr/bin/env bb
;; ugachi 穿ち — stewardship-ledger kotoba Datom-log writer (content-addressed).
(ns ugachi.methods.kotoba
  "kotoba.cljc — ugachi 穿ち append-only STEWARDSHIP LEDGER writer
  (ADR-2606170900, on ADR-2605262130 + ADR-2605312345; same content-addressed
  commit-DAG machinery as the meisai/kakaku family).

  Persists the §2(l) gate VERDICTS to a local append-only EDN log of
  content-addressed transactions — the 'stewardship ledger' ugachi's report names.
  Each tx CID = 'b' + sha256-hex over the canonical JSON
  {\"datoms\":[…],\"prev\":…} (keys stay ':…' STRINGS; EAVT = [op entity attr value],
  op :db/add only). prev-cid chaining makes the log tamper-evident (verify-chain).
  Deterministic: caller supplies tx-id + as-of (no wall clock) → resume-safe.
  ...
  (a stewardship record, NEVER a target-list — the data is gate decisions, not
  mine locations)."
   ...
   (do
     (defn append-tx [tx log-path]
       (let [f (io/file log-path)]
         ...
         (when-not (.exists f)
           (spit f (str ";; ugachi 穿ち — STEWARDSHIP LEDGER (append-only, content-addressed "
                        "EAVT commit-DAG of §2(l) gate verdicts). Generated; DO NOT hand-edit. "
                        "A record of extraction DECISIONS, never a target-list. ADR-2606170900.\n")))
